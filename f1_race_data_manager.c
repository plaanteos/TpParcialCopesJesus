...
int comparar_tiempos(const void *a, const void *b) {
    ...
    // Caso especial: si algún tiempo es 0, debe ir al final (descalificado)
...
void mostrar_tiempos() {
    ...
    // Reservar memoria para los tiempos
    ...
    // Leer y almacenar los tiempos en un arreglo dinámico
    while (fread(&tiempo, sizeof(Tiempo), 1, dat) == 1) {
    ...
    // Mostrar la clasificación de la vuelta 0
    printf("\nClasificación Vuelta 0 (Clasificatoria):\n");
    printf("------------------------------------------\n");
    printf("Puesto | Número Corredor | Tiempo\n");
    printf("------------------------------------------\n");
    
    // Filtrar y ordenar los tiempos de la vuelta 0
    Tiempo *tiempos_vuelta0 = malloc(contador * sizeof(Tiempo));
    if (tiempos_vuelta0 == NULL) {
        printf("Error: No se puede asignar memoria para tiempos de vuelta 0.\n");
        free(tiempos);
        return;
    }
    
    int contador_vuelta0 = 0;
    for (int i = 0; i < contador; i++) {
        if (tiempos[i].numero_vuelta == 0) {
            tiempos_vuelta0[contador_vuelta0++] = tiempos[i];
        }
    }
    
    if (contador_vuelta0 == 0) {
        printf("No hay tiempos registrados para la vuelta clasificatoria.\n");
    } else {
        // Ordenar los tiempos de la vuelta 0
        qsort(tiempos_vuelta0, contador_vuelta0, sizeof(Tiempo), comparar_tiempos);
        
        for (int i = 0; i < contador_vuelta0; i++) {
            int minutos = (int)(tiempos_vuelta0[i].tiempo / 60);
            float segundos = tiempos_vuelta0[i].tiempo - (minutos * 60);
            printf("%6d | %16d | %d:%.3f\n", 
                   i + 1, 
                   tiempos_vuelta0[i].numero_corredor, 
                   minutos, 
                   segundos);
        }
    }
    
    free(tiempos_vuelta0);
    esperar(5);
    
    // Mostrar los tiempos de las demás vueltas
    printf("\nTiempos de las demás vueltas:\n");
    printf("------------------------------------------\n");
    printf("Vuelta | Número Corredor | Tiempo\n");
    printf("------------------------------------------\n");
    
    int tiempos_otras_vueltas = 0;
    for (int i = 0; i < contador; i++) {
        if (tiempos[i].numero_vuelta != 0) {
            ...
            printf("%6d | %16d | %d:%.3f\n", 
                   ...
                   minutos, 
                   segundos);
            tiempos_otras_vueltas++;
        }
    }
    
    if (tiempos_otras_vueltas == 0) {
        printf("No hay tiempos registrados para otras vueltas.\n");
    }
